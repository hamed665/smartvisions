...
function existingToDiscovered(row: ExistingBusiness, fallbackCountry: string, fallbackCity: string): DiscoveredBusiness {
  return {
    ...
    retrievedAt: row.google_intelligence_retrieved_at ?? new Date().toISOString(),
  };
}
...
export async function qualifyGooglePlacesPriorityBatch(form: FormData) {
  ...
  try {
    ...
    if (integration?.status !== 'CONNECTED' || integration.enabled !== true) {
      throw new Error('Google Places must be CONNECTED before batch qualification');
    }

    const { data: discoveries, error: discoveryError } = await ctx.supabase
    ...
    if (!candidates.length) {
      ...
      for (const row of candidates) {
        ...
        let business: DiscoveredBusiness;
        ...
        if (priorityQualified) {
          ...
          if (!leadId) {
            const { data: insertedLead, error: insertLeadError } = await ctx.supabase
              .from('leads')
              .insert(buildLeadPersistenceRow(ctx.organizationId, businessId, business))
              .select('id')
              .single();
            if (insertLeadError) {
        ...
        const { error: updateError } = await ctx.supabase.from('discovery_records').update({
          raw_payload: {
            ...raw,
      ...
      const { error: auditError } = await ctx.supabase.from('audit_logs').insert({
        ...
        actor_type: 'USER',
        actor_id: ctx.userId,
        action: 'GOOGLE_PLACES_PRIORITY_BATCH_QUALIFICATION',
    ...
    await ctx.supabase.from('audit_logs').insert({
      ...
      actor_type: 'USER',
      actor_id: ctx.userId,
      action: 'GOOGLE_PLACES_PRIORITY_BATCH_QUALIFICATION_FAILED',
      entity_type: 'integration',
      entity_id: ctx.organizationId,
