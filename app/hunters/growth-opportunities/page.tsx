...
type Opportunity = {
  ...
  cheapest_next_action: 'SKIP'|'CONTACT_READY'|'SOCIAL_CHECK'|'WEBSITE_EVIDENCE';
...
export default async function GrowthOpportunitiesPage(){
  ...
  return <div>
    ...
    <section className="grid"><div className="card"><span className="muted">Routed businesses</span><div className="value">{rows.length}</div></div><div className="card"><span className="muted">Contact-ready now</span><div className="value">{contactReady}</div></div><div className="card"><span className="muted">Website opportunities</span><div className="value">{websiteHigh}</div></div><div className="card"><span className="muted">Paid evidence candidates</span><div className="value">{paidEvidence}</div></div></section>

    <section className="panel"><div className="headerRow"><div><h2>Zero-cost cached routing</h2><p className="muted">Re-route cached businesses, rebuild priority and message context, and decide whether the next useful action costs money. This action itself calls no provider.</p></div><form action={routeCachedGrowthOpportunities}><button disabled={role!=='OWNER'}>Route cached businesses</button></form></div></section>

    <section className="panel"><h2>Cost-aware personalization rules</h2><div className="healthList"><span>Priority score <strong>contactability + need + service fit + revenue potential</strong></span><span>No-site + contact-ready <strong>contact without buying more evidence</strong></span><span>Social/website evidence <strong>only when it can change the offer</strong></span><span>Unknown evidence <strong>stays unknown; no invented weakness</strong></span><span>AI personalization <strong>deferred to sales stage, not acquisition</strong></span></div></section>

    {lanes.map(lane=>{const items=rows.filter(row=>row.sales_lane===lane);return <section className="panel" key={lane}><div className="headerRow"><div><h2>{laneTitle(lane)}</h2><p className="muted">{items.length} routed business(es), ordered by zero-cost personalization priority.</p></div><span className="pill">{lane}</span></div><div className="settingsList">{items.length===0?<p className="muted">No businesses routed here yet.</p>:items.map(row=>{const business=Array.isArray(row.businesses)?row.businesses[0]:row.businesses;return <div className="settingsRow" key={row.id}><div><strong>{business?.name??'Business'}</strong><div className="muted">{business?.city??'city pending'} · {row.website_class} website · priority {row.personalization_priority_score}</div><div className="muted">{business?.phone??'phone pending'} · {business?.formatted_address??'address pending'}</div><div className="muted">Bundle: {(row.offer_bundle??row.recommended_services??[]).join(' + ')||'review required'}</div><div className="muted">Angle: {row.recommended_angle??'generic growth review'}</div><div className="muted">Next: {actionLabel(row.cheapest_next_action)} {row.next_action_can_spend_money?'· may spend after Cost Guard':'· $0 provider spend'}</div><div className="muted">Why: {row.next_action_reason??'No next-action reason recorded'}</div><div className="muted">Fingerprint: {(row.personalization_fingerprint??[]).join(' · ')||'pending'}</div><div>{business?.whatsapp?<a className="textLink" href={business.whatsapp} target="_blank" rel="noreferrer">WhatsApp candidate ↗</a>:null}{business?.google_maps_uri?<> · <a className="textLink" href={business.google_maps_uri} target="_blank" rel="noreferrer">Maps ↗</a></>:null}</div></div><div><strong>Priority {row.personalization_priority_score}</strong><div className="muted">Contact {row.contactability_score} · Need {row.need_score}</div><div className="muted">Fit {row.service_fit_score} · Revenue {row.revenue_potential_score}</div><div className="muted">Web {row.website_score} · Local {row.local_content_score} · AI {row.ai_content_score}</div></div></div>;})}</div></section>;})}
  </div>;
}
