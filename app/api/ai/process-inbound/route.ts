...
export async function POST(request: Request) {
  ...
  const body = await request.json() as { context?: AgentContext; agentsPaused?: boolean; idempotencyKey?: string };
  ...
  let { data: existing, error: existingError } = await readExisting();
  ...
  const { data: claimed, error: claimError } = await supabase.from('agent_runs').insert({
    ...
    input_message: body.context.message,
    routed_agents: [],
    status: 'PROCESSING',
    trace: {},
