...
export async function POST(request: Request) {
  ...
  try {
    ...
    const outreach = await supabase.from('outreach_messages').upsert({ organization_id: body.organizationId, lead_id: message.lead_id, campaign_id: campaign.id, mailbox_id: mailboxId, channel: 'EMAIL', direction: 'OUTBOUND', status: 'SENT', provider_message_id: providerMessageId, idempotency_key: idempotencyKey, subject, body: message.original_text, sent_at: nowIso, metadata: { marketCode, campaignId: campaign.id, source: 'CONTROLLED_MULTI_MARKET_AUTOPILOT' } }, { onConflict: 'organization_id,provider_message_id' });
