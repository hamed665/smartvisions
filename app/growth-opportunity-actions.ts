...
const toDiscovered = (row: CachedBusiness): DiscoveredBusiness => ({
  ...
  whatsapp: row.whatsapp ?? deriveWhatsappCandidate(row.international_phone, row.phone, row.country_code),
...
export async function routeCachedGrowthOpportunities() {
  ...
  let destination = '/hunters/growth-opportunities';
  try {
    const { data, error } = await ctx.supabase
      .from('businesses')
      .select('id,name,country_code,city,category,google_place_id,official_website,phone,international_phone,whatsapp,formatted_address,google_maps_uri,google_rating,google_user_rating_count,google_business_status')
    ...
    for (const row of (data ?? []) as CachedBusiness[]) {
      ...
      const opportunity = buildGrowthOpportunity(business);
      const { error: upsertError } = await ctx.supabase.from('growth_opportunities').upsert({
    ...
    const { error: auditError } = await ctx.supabase.from('audit_logs').insert({
      ...
      actor_type: 'USER',
      actor_id: ctx.userId,
      action: 'ROUTE_CACHED_GROWTH_OPPORTUNITIES',
      entity_type: 'growth_opportunity',
      entity_id: ctx.organizationId,
      after_data: { routed, skipped, providerCalls: 0, llmCalls: 0, socialApiCalls: 0, reviewsFetched: false, outreachTriggered: false },
    });
    if (auditError) throw auditError;
    destination = `/hunters/growth-opportunities?routed=${routed}&skipped=${skipped}`;
  } catch (error) {
    const message = error instanceof Error ? error.message.slice(0, 200) : 'Cached growth routing failed';
    destination = `/hunters/growth-opportunities?error=${encodeURIComponent(message)}`;
  }
