...
export async function runDeterministicWebsiteAudit(form: FormData) {
  const ctx = await getCurrentOrganization(true); const leadId = String(form.get('leadId') ?? '').trim(); if (!leadId) throw new Error('leadId is required'); let destination = `/leads/${encodeURIComponent(leadId)}`;
  ...
  if (leadError) throw leadError; if (!lead?.business_id) throw new Error('Lead has no business to audit');
  const business = Array.isArray(lead.businesses) ? lead.businesses[0] : lead.businesses; const website = String(business?.official_website ?? '').trim(); if (!website) throw new Error('Business has no official website');
  ...
  const { data: cached, error: cacheError } = await ctx.supabase.from('website_audits').select('id,audited_at,status').eq('organization_id', ctx.organizationId).eq('business_id', lead.business_id).eq('status', 'SUCCEEDED').gte('audited_at', cacheCutoff).order('audited_at', { ascending: false }).limit(1).maybeSingle();
  if (cacheError) throw cacheError; if (cached) redirect(`/leads/${encodeURIComponent(leadId)}?audit=cached`);
  ...
  const { data: auditRow, error: insertError } = await ctx.supabase.from('website_audits').insert({ organization_id: ctx.organizationId, business_id: lead.business_id, source_url: website, status: 'RUNNING' }).select('id').single(); if (insertError) throw insertError;
  try {
    ...
    const { error: leadUpdateError } = await ctx.supabase.from('leads').update({ status: lead.status === 'NEW' ? 'AUDITED' : lead.status, updated_at: completedAt }).eq('organization_id', ctx.organizationId).eq('id', leadId); if (leadUpdateError) throw leadUpdateError;
    const { error: logError } = await ctx.supabase.from('audit_logs').insert({ organization_id: ctx.organizationId, actor_type: 'USER', actor_id: ctx.userId, action: 'DETERMINISTIC_WEBSITE_AUDIT', entity_type: 'website_audit', entity_id: auditRow.id, after_data: { leadId, businessId: lead.business_id, sourceUrl: fetched.finalUrl, bytesInspected: fetched.html.length, llmUsed: false, paidProviderUsed: false, outreachTriggered: false } }); if (logError) throw logError;
    destination = `/leads/${encodeURIComponent(leadId)}?audit=success`;
  } catch (error) {
    ...
    await ctx.supabase.from('audit_logs').insert({ organization_id: ctx.organizationId, actor_type: 'USER', actor_id: ctx.userId, action: 'DETERMINISTIC_WEBSITE_AUDIT_FAILED', entity_type: 'website_audit', entity_id: auditRow.id, after_data: { leadId, businessId: lead.business_id, error: message, outreachTriggered: false } });
    destination = `/leads/${encodeURIComponent(leadId)}?audit=error&message=${encodeURIComponent(message)}`;
  }
  revalidatePath('/leads'); revalidatePath(`/leads/${leadId}`); revalidatePath('/hunters/google-places'); redirect(destination);
}
