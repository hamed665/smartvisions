...
export async function GET(request: Request) {
  ...
  const missing = REQUIRED_KEYS.filter((key) => !values[key]?.trim());
  if (!String(values.META_WHATSAPP_ACCESS_TOKEN ?? values.META_WHATSAPP_TOKEN ?? values.WHATSAPP_ACCESS_TOKEN ?? '').trim()) {
    missing.push('META_WHATSAPP_ACCESS_TOKEN' as (typeof REQUIRED_KEYS)[number]);
  }
  if (!String(values.META_WHATSAPP_PHONE_NUMBER_ID ?? values.WHATSAPP_PHONE_NUMBER_ID ?? '').trim()) {
    missing.push('META_WHATSAPP_PHONE_NUMBER_ID' as (typeof REQUIRED_KEYS)[number]);
  }
