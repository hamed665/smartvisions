...
export default async function ReportsPage(){
  ...
  return <div>
    ...
    <section className="panel">
      ...
      <div className="healthList"><span>Sent <strong>{catalog.sent}</strong></span><span>Delivered <strong>{catalog.delivered}</strong></span><span>Read <strong>{catalog.read}</strong></span><span>Replied <strong>{catalog.replied}</strong></span><span>Human handoff <strong>{catalog.handoff}</strong></span><span>Currently won after catalog exposure <strong>{catalog.won}</strong></span></div>
      ...
      <p className="muted">No fake click metric is shown because the current Meta webhook evidence does not provide a reliable product-view/click event. *Won means a Lead is currently WON and had a prior catalog send; it is deliberately not presented as a timestamped click-to-win claim.</p>
    </section>
