...
export function calculateNoWebsiteOpportunityScore(business: DiscoveredBusiness) {
  ...
  if (websiteClass === 'NONE') {
    score += 40;
    reasons.push('No standalone website returned by Google');
  } else if (websiteClass === 'CONTACT_ONLY') {
    score += 35;
    reasons.push('Only a social/contact link is present; no standalone website');
  }
  if (business.phone || business.internationalPhone) {
    score += 10;
    reasons.push('Direct phone contact is available');
  }
  if (business.whatsapp || deriveWhatsappCandidate(business.internationalPhone, business.phone, business.countryCode)) {
    score += 5;
    reasons.push('WhatsApp candidate link can be generated without another API call');
  }
  ...
  if (reviewCount >= 100) {
    ...
    score += 2;
    reasons.push('Google ratings confirm customer activity');
  }
...
export function buildBusinessPersistenceRow(organizationId: string, business: DiscoveredBusiness) {
  ...
  return {
    ...
    dedupe_domain: hasStandaloneWebsite(business) ? normalizeDomain(business.officialWebsite) ?? null : null,
