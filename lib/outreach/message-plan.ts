...
const CANONICAL_OFFER_COPY: Record<string, CanonicalOfferCopy> = {
  business_website: { english: 'a clear standalone business website', arabic: 'موقع أعمال مستقل وواضح' },
  premium_bilingual_website: { english: 'a focused bilingual website upgrade', arabic: 'تطوير مركز للموقع باللغتين' },
  custom_website: { english: 'a tailored website upgrade', arabic: 'تطوير مخصص للموقع' },
  seo_growth: { english: 'targeted SEO improvements', arabic: 'تحسينات مركزة للظهور في محركات البحث' },
  muscat_content_production: { english: 'focused on-site content production', arabic: 'إنتاج محتوى ميداني بشكل مركز' },
  ai_reels_4: { english: 'a focused AI reels package', arabic: 'باقة ريلز مركزة بالذكاء الاصطناعي' },
  ai_reels_8: { english: 'a focused AI reels package', arabic: 'باقة ريلز مركزة بالذكاء الاصطناعي' },
  whatsapp_ai_setup: { english: 'a practical WhatsApp AI setup', arabic: 'إعداد واتساب ذكي بشكل عملي' },
};
...
function canonicalObservation(sourceEvidence: string): CanonicalFirstTouchObservation | null {
  ...
  ) return { key: 'NO_STANDALONE_WEBSITE', sourceEvidence: evidence, english: 'your current public web presence appears to rely mainly on directory, social, or contact pages rather than a standalone website', arabic: 'حضوركم الحالي على الويب ظاهر بشكل أساسي عبر صفحات دليل أو تواصل أو سوشال بدل موقع مستقل' };
  ...
  if (brokenLinks) {
    ...
    return { key: 'BROKEN_LINKS', sourceEvidence: evidence, english: `your website audit detected ${count} broken link${count === '1' ? '' : 's'}`, arabic: `فحص موقعكم رصد ${count} رابط${count === '1' ? 'اً' : ''} لا يعمل` };
  }
...
export function buildMessagePlan(input: MessagePlanInput) {
  ...
  const bilingualGulf = GULF_MARKETS.has(input.marketCode);
  const singleLanguage = chooseLanguage({ marketCode: input.marketCode, detectedLanguage: input.detectedLanguage, preferredLanguage: input.preferredLanguage });
  const languages = bilingualGulf ? ['en', locale.primaryLocale] : [singleLanguage];
  const language = bilingualGulf ? `en+${locale.primaryLocale}` : singleLanguage;
  ...
  return {
    ...
    language,
    languages,
    languageMode: bilingualGulf ? 'BILINGUAL_FIRST_TOUCH' as const : 'SINGLE_LANGUAGE' as const,
    dialect: locale.dialect,
    tone: locale.tone,
    dialectIntensity: locale.dialectIntensity,
    maxWords: locale.maxFirstTouchWords,
    maxWordsPerLanguage: bilingualGulf ? Math.max(20, Math.floor(locale.maxFirstTouchWords / 2)) : locale.maxFirstTouchWords,
    evidence,
    recommendedOffer: input.recommendedOffer,
    structure: bilingualGulf ? ['english_personal_observation_and_relevant_value','localized_arabic_equivalent_with_same_facts','one_shared_low_pressure_cta'] : ['personal_observation','one_relevant_problem_or_opportunity','one_relevant_solution','one_short_benefit','low_pressure_cta'],
    compositionRules: bilingualGulf ? ['English section first, localized Gulf Arabic section second.','Both sections must communicate the same verified facts and offer.','Keep both sections concise and low pressure.','After a reply, mirror the customer language.'] : ['Use the planned single language and market tone.'],
...
function localizedEnglishOpening(marketCode: MarketCode, businessName: string) {
  if (marketCode === 'GB') return `Hi ${businessName}`;
  if (marketCode === 'US' || marketCode === 'CA') return `Hi ${businessName}`;
  return `Hi ${businessName}`;
}

export function buildCanonicalFirstTouchDraft(input: MessagePlanInput) {
  const observation = input.evidence.filter(Boolean).map(canonicalObservation).find((item): item is CanonicalFirstTouchObservation => item !== null);
  ...
  const english = `${localizedEnglishOpening(input.marketCode, businessName)}, I noticed ${observation.english}. Based on that, ${offer.english} could be useful. Would it help if I sent you a short outline?`;
  if (GULF_MARKETS.has(input.marketCode)) {
    const arabic = `هلا ${businessName}، لاحظنا إن ${observation.arabic}. وبناءً على هالمعلومة، ممكن نساعدكم من خلال ${offer.arabic}. إذا حابين، نرسل لكم ملخص قصير؟`;
    return { plan, observation, offer, sections: { english, arabic }, text: `${english}\n\n${arabic}`, subject: `A quick idea for ${businessName}` };
  }
  return { plan, observation, offer, sections: { english }, text: english, subject: `A quick idea for ${businessName}` };
}
...
export function buildOmanFirstTouchDraft(input: MessagePlanInput) {
  ...
  return buildCanonicalFirstTouchDraft(input);
}
