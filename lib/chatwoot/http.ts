...
function platformToken() {
  const token = normalizeChatwootAccessToken(
...
function authToken(auth: ChatwootAuth) {
  ...
  const token = normalizeChatwootAccessToken(auth.accessToken);
  if (!token) {
    throw new ChatwootHttpError({
      code: 'CONFIG_INVALID',
      message: 'Chatwoot user access token is invalid',
    });
  }
...
function serializeRequestBody(body: unknown) {
  ...
  try {
    ...
    if (value === undefined) {
      throw new Error('undefined JSON result');
    }
    serialized = value;
  } catch {
    throw new ChatwootHttpError({
  ...
  if (bytes > MAX_REQUEST_BYTES) {
    throw new ChatwootHttpError({
      ...
      message: 'Chatwoot request exceeded the configured size limit',
    });
  }
...
async function cancelResponseBody(response: Response) {
  ...
  try {
    ...
    // Body disposal is best-effort only. Never mask the actual HTTP error.
  }
}
...
export async function chatwootProvisioningRequest<T>(input: {
  ...
  const requestBody = serializeRequestBody(input.body);
    ...
    try {
      const response = await fetchImpl(url, {
        ...
        headers: {
          ...
          'X-SmartVisions-Request-Id': requestId,
        },
        body: requestBody,
        cache: 'no-store',
        redirect: 'error',
      ...
      throw new ChatwootHttpError({
        ...
        retryable: isChatwootReadMethod(method) && networkFailure,
        ambiguousMutationOutcome: mutationOutcomeMayBeAmbiguous({
          method,
          networkFailure: true,
        }),
