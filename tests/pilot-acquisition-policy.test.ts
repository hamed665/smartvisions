...
describe('time-boxed Oman pilot acquisition policy', () => {
  ...
  it('self-closes when the paid qualification cap is reached', () => {
    const result = evaluatePilotAcquisitionPolicy({ ...base, qualificationCount: 3 });
    expect(result.allowed).toBe(false);
    expect(result.terminal).toBe(true);
  ...
  it('caps configuration attempts above three paid qualifications', () => {
    const result = evaluatePilotAcquisitionPolicy({
      ...base,
      config: { ...baseConfig, maxPaidQualifications: 99 },
