...
drop trigger if exists chatwoot_bridge_command_claims_command_guard
  on public.chatwoot_bridge_command_claims;
create trigger chatwoot_bridge_command_claims_command_guard
...
begin
  ...
  if not found then raise exception 'communication binding not found'; end if;
...
begin
  ...
  if not v_claim.is_new then
    ...
    if not found then raise exception 'Chatwoot Account mapping not found'; end if;
