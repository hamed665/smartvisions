...
begin
  ...
  if new.default_text is not null then
    ...
    then
      ...
      and new.default_text !~* '^https?://[^\s]+$'
    then
      raise exception 'URL default must use http/https';
    end if;
...
for select to authenticated
using (
  public.crm_custom_field_can_read_definition(organization_id, definition_id)
);
