...
- The mobile onboarding layout fix from PR #236 is already running on the VPS using the successful exact PR-head immutable image while the canonical exact-main Source Image #37 finishes rebuilding. Runtime verification confirms the responsive mobile row/select source, Community-only provenance, S3 Active Storage, healthy Puma/Sidekiq/Redis, and no `enterprise/` tree.
