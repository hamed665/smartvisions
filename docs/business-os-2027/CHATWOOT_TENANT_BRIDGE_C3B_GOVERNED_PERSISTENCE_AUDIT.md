...
Canonical role resolver found in current code: `lib/business-os/control-plane.ts` exports `effectiveRoleForScope`, `assertTenantScopeLineage` and `isScopeAssignmentApplicable`. It returns OWNER only for Organization OWNER; otherwise it resolves the deepest applicable scoped assignment and ABAC attributes, falling back to Organization role. A future AccountUser writer must use this resolver or a demonstrably equivalent database check on fresh canonical rows. For a Business-wide Account membership, the target lineage must include Organization, Brand and tenant Business; a BRANCH/TEAM assignment is not automatically Business-wide authorization. Decide and test Inbox-restricted access separately before projecting narrower staff into an Account. The current C3B Account-only path has no AccountUser writer.
